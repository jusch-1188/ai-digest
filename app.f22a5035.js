...
//  and inline-handler CSP violations. Runs after body parse.
// ═══════════════════════════════════════════════════════════
(function () {
...
function addDays(dateStr, n) {
  // Parse as UTC so toISOString() round-trips correctly in any timezone
...
function renderGrid() {
  ...
  // Apply filters then sort newest → oldest (stable within the same date)
  ...
  if (state.allCards.length === 0) {
    const emptyLabel = {
      ...
      yesterday:    `No data for yesterday (${fmtDate(addDays(todayStr(), -1))})`,
    ...
    const emptyHint = (state.period === 'today' || state.period === 'yesterday')
  ...
  grid.innerHTML = visible.map(renderCard).join('') + (hasMore ? `
...
// ═══════════════════════════════════════════════════════════
//  Load more (event delegation — avoids inline handlers)
// ═══════════════════════════════════════════════════════════
document.getElementById('cardGrid').addEventListener('click', e => {
  if (e.target.closest('[data-action="load-more"]')) {
    state.visibleCount += PAGE_SIZE;
    renderGrid();
    // Keep the user's scroll position — don't jump to top
  }
});
