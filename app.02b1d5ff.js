...
async function loadIndex() {
  ...
  try {
    ...
    const res  = await fetch('/data/index.json', { signal: ctrl.signal });
    clearTimeout(tid);
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    const idx  = await res.json();
    availableDates = Array.isArray(idx.dates) ? idx.dates : [];
  } catch {
    availableDates = []; // graceful degradation — skip manifest, try nothing
  }
...
function applyFilters(cards) {
  ...
  if (state.model !== 'all') {
    ...
    out = out.filter(c => keywords.some(kw => c._text.includes(kw)));
  }
